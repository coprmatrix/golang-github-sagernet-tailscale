...
for i in go.mod go.sum
do
  sed -i "s~github.com/tailscale/xnet~~g" "${i}"
done
...
do
  if test -f ${i}
  then
    sed -i "s~github.com/tailscale/xnet~golang.org/x/net~g;" "${i}"
  fi
done
...
do
  if test -f ${i}
  then
    sed -i "s~github.com/illarion/gonotify/v2~github.com/illarion/gonotify/v3~g;" "${i}"
  fi
done
